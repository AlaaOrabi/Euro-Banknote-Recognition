...
def normalize_image(image_path):
    ...
    image = image.resize((800, 600), Image.LANCZOS)  # Resize to a standard size
...
def preprocess():
    ...
    strip_metadata(image_path)  # Strip metadata from the image
    normalize_image(image_path)  # Normalize the image
...
def recognize():
    ...
    strip_metadata(test_image_path)  # Strip metadata from the image
    normalize_image(test_image_path)  # Normalize the image
    ...
    try:
        ...
        # Compress the image to ensure it does not exceed 200 KB
        quality = 95
        while os.path.getsize(result_image_path) > 200 * 1024 and quality > 10:
            cv2.imwrite(result_image_path, result_image_resized, [int(cv2.IMWRITE_JPEG_QUALITY), quality])
            quality -= 5
